...
export default function UsersPage() {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const data = await response.json() as { users: Array<Omit<DisplayUser, 'role'> & { role: 'AGENT' | 'USER' }> };
      setAllUsers(data.users.map((user) => ({ ...user, role: user.role === 'USER' ? 'CUSTOMER' : user.role })));
    } catch (error) {
  ...
  const agentCount = allUsers.filter((user) => user.role === 'AGENT').length;
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {loading && (
                  <tr><td colSpan={4} className="px-5 py-8 text-center text-sm text-gray-500">Loading users...</td></tr>
                )}
                {filteredUsers.map((user, index) => {
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <span className="text-sm text-gray-600">{user.status.replace('_', ' ')}</span>
                      </td>
                      <td className="px-5 py-3.5">
