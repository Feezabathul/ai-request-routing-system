...
export default function AgentsPage() {
  ...
  const totalAssigned = agents.reduce((a, b) => a + b.assignedRequests, 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {agents.length === 0 ? (
              <p className="text-sm text-gray-500">No agents to display</p>
            ) : (
              Array.from(new Set(agents.flatMap((a) => a.skills))).map((skill) => {
                const count = agents.filter((a) => a.skills.includes(skill)).length;
                return (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              { label: 'Average Response Time', value: agents.length === 0 ? '—' : '3.8 min', color: 'indigo' },
              { label: 'Average Rating', value: agents.length === 0 ? '—' : '4.73 / 5', color: 'amber' },
              { label: 'Requests Resolved Today', value: agents.length === 0 ? '0' : '49', color: 'emerald' },
            ].map((metric) => (
